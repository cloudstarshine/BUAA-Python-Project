...
class StatisticBar:
    def __init__(self, master, user_name, is_admin):
        ...
        # 设置子窗口
        self.statistic_bar = Frame(self.master, width=1000, height=100)
        self.statistic_bar.pack()
    ...
    def set_button(self):
        ...
        # 返回
        self.button_exit = Button(self.statistic_bar, text='返回', command=self.cmd_exit, font=word_set)
    ...
    def set_bar(self):
        ...
        bar_width = 0.3
        index_right = npy.arange(len(total_num))
        index_total = index_right + bar_width

        plt.bar(index_right, height=right_num, width=bar_width, color='b', label='Right Number')
        plt.bar(index_total, height=total_num, width=bar_width, color='r', label='Total Number')

        plt.legend()
        plt.xticks(index_right + bar_width / 2, question)
        plt.ylabel('Number')
